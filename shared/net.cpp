// Otium Code Shared Lib Net v1.10.0
// Original version from MayFly AI net.cpp
// - 2026/07/02 (1.10.0) - Add flag to socket address ToString so it is possible to get string without port
...
std::string GetComputerId() {
	...
	DWORD lBuffer=sizeof(name);
	if(GetComputerName(name,&lBuffer))
...
#ifdef __GNUC__
	int iRet = GetComputerName(szHostName, &cbHostName);
#else
	int iRet = GetComputerNameExA(ComputerNamePhysicalDnsHostname, szHostName, &cbHostName);
#endif
	if(iRet)
		return szHostName;
	return "";
...
Socket::Socket(AddressFamily af,SocketType st,ProtocolType pt) {
	...
	if((int)fd==INVALID_SOCKET)return;
...
#ifdef _WIN32
void Socket::SetTimeout(long secs,long u_secs) {
    DWORD dw=(secs*1000)+((u_secs+999)/1000);
    setsockopt(m_handle->m_socket,SOL_SOCKET,SO_RCVTIMEO,(char*)&dw,sizeof(dw));
}
#else
void Socket::SetTimeout(long secs,long u_secs) {
	struct timeval tv;
	tv.tv_sec=secs;
	tv.tv_usec=u_secs;
	if (setsockopt(m_handle->m_socket,SOL_SOCKET,SO_RCVTIMEO,&tv,sizeof(tv))<0) {
		FATAL("Socket::SetTimeout");
	}
}
#endif
...
std::string SocketAddress::ToString(bool includePort)const {
	sockaddr_in* sa=(sockaddr_in*)&m_handle;
	if(!sa->sin_port)
		return "0.0.0.0:0";
	uint8_t* p=GetIPBytes();
	if(!includePort)
		return stdx::format_string("%d.%d.%d.%d",p[0],p[1],p[2],p[3]);
...
bool SocketServer::Begin(const char* bindAddress,uint16_t bindPort,int timeout) {
	if(m_threadRunning) {
		uprintf("ERROR: SocketServer::Begin thread allready running\n");
		return false;
	}
	...
	uint64_t t=GetTimeEpochMilliseconds();
	while(!m_threadRunning) {
		std::this_thread::sleep_for(std::chrono::milliseconds(10));
		if((int)(GetTimeEpochMilliseconds()-t)>timeout) {
			m_close=true;
			SetEvent();
			m_listenThread.join();
			return false;
...
void SocketServer::End() {
	if(!m_threadRunning) {
		uprintf("ERROR: SocketServer::End thread not running\n");
		return;
	}
	m_close=true;
	SetEvent();
...
int SocketServer::Run() {
	...
	while(1) {
		if(socketMaster.Bind(address))
			break;
		//SOCKETSERVERWARNING("WARNING: %s::Run Unable to bind '%s'\n",ClassName(),address.ToString().c_str());
...
void SocketClient::End() {
	if(!m_running)
		return;
	m_close=true;
	//if(m_socketSet.m_SignalSocket.m_handle)
	//	return;
	m_socketSet.SignalSelect();
		...
		if(!m_connected) {
			...
			if(m_nonBlockConnect) {
				...
				if(!m_socket->Connect(m_address)) {
					SOCKETCLIENTNOTIFY("NOTIFY: %s::KeepAliveThreadFunc Unable to connect to %s. Retrying in 5 seconds\n",InstanceName(),m_address.ToString().c_str());
		...
		if(numSocketsReady>0) {
			...
			if(m_socketSet.IsWrite(*m_socket)) {
				...
				if(wasSentByteSize<=0 || m_socket->m_send.DataByteSize()==0) {
					if(wasSentByteSize<=0|| m_socket->m_closeAfterSend) {
						...
						m_socket=0;
						bool reconnect;
						TIMEGUARD(reconnect=OnClose(),20*1000,"WARNING: KeepAliveThreadFunc OnClose\n");
						m_connected=false;
						//Thread::Sleep(1000);
			...
			if(m_socketSet.IsRead(*m_socket)) {
				...
				if(close) {
					...
					m_socket=0;
					bool reconnect;
					TIMEGUARD(reconnect=OnClose(),20*1000,"WARNING: KeepAliveThreadFunc::Run OnClose\n");
					m_connected=false;
					if(!reconnect)
						break;
					//Thread::Sleep(1000);
			...
			if(numSocketsReady<0) {
				...
				m_socket=0;
				bool reconnect;
				TIMEGUARD(reconnect=OnClose(),20*1000,"WARNING: SocketClient::KeepAliveThreadFun OnClose\n");
				m_connected=false;
...
int lort1() {
	...
	if((dwRetVal=GetAdaptersInfo(adapterInfo, &ulOutBufLen))==NO_ERROR) {
		adapter=adapterInfo;
		while (adapter) {
			uprintf("\tComboIndex: \t%d\n", adapter->ComboIndex);
			uprintf("\tAdapter Name: \t%s\n", adapter->AdapterName);
			uprintf("\tAdapter Desc: \t%s\n", adapter->Description);
			uprintf("\tAdapter Addr: \t");
			for (i=0; i < adapter->AddressLength; i++) {
				if(i==(adapter->AddressLength - 1))
					uprintf("%.2X\n", (int) adapter->Address[i]);
				else
					uprintf("%.2X-", (int) adapter->Address[i]);
			}
			uprintf("\tIndex: \t%d\n", adapter->Index);
			uprintf("\tType: \t");
			switch (adapter->Type) {
			...
			if(adapter->DhcpEnabled) {
				uprintf("\tDHCP Enabled: Yes\n");
				uprintf("\t  DHCP Server: \t%s\n",
						adapter->DhcpServer.IpAddress.String);

				uprintf("\t  Lease Obtained: ");
				// Display local time
				error=_localtime32_s(&newtime, (__time32_t*) &adapter->LeaseObtained);
				if(error)
					uprintf("Invalid Argument to _localtime32_s\n");
				else {
					// Convert to an ASCII representation
					error=asctime_s(buffer, 32, &newtime);
					if(error)
						uprintf("Invalid Argument to asctime_s\n");
					else
						// asctime_s returns the string terminated by \n\0
						uprintf("%s", buffer);
				}

				uprintf("\t  Lease Expires:  ");
				error=_localtime32_s(&newtime, (__time32_t*) &adapter->LeaseExpires);
				if(error)
					uprintf("Invalid Argument to _localtime32_s\n");
				else {
					// Convert to an ASCII representation
				...
				uprintf("\tDHCP Enabled: No\n");

			if(adapter->HaveWins) {
				uprintf("\tHave Wins: Yes\n");
				uprintf("\t  Primary Wins Server:    %s\n",
						adapter->PrimaryWinsServer.IpAddress.String);
				uprintf("\t  Secondary Wins Server:  %s\n",
						adapter->SecondaryWinsServer.IpAddress.String);
			} else
				uprintf("\tHave Wins: No\n");
			adapter=adapter->Next;
			uprintf("\n");
		}
	} else {
		uprintf("GetAdaptersInfo failed with error: %d\n", dwRetVal);
