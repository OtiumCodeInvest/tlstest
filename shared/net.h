...
class SocketAddress {
	...
	public:
		...
		SocketAddress(const std::string& address){Set(address.c_str());}
		...
		void Set(const std::string& address){Set(address.c_str());}
...
class SocketServer {
	public:
		...
		std::string BindAddress()const{return m_bindAddress;}
		int BindPort()const{return m_bindPort;}

		virtual bool Begin(const char* bindAddress,uint16_t bindPort,int timeout);
		virtual void End();
		virtual int Run();
